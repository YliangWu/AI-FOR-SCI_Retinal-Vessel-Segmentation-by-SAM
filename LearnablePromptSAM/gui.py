...
def medsam_inference(medsam_model, img_embed, box_1024, height, width):
    low_res_pred=img_embed[:,1,:,:].unsqueeze(0)
...
# set up model
checkpoint = "./weights/sam_vit_b_01ec64.pth" #源模型文件
new_CKPT_PATH="./weights/sam_vit_b_prompt.pth" #最新模型文件
model_name = "vit_b"
num_classes=2
#注册一个原模型
LPsam = PromptSAM(model_name, checkpoint=checkpoint, num_classes=num_classes, reduction=4, upsample_times=2, groups=4)
#加载checkpoint参数
checkpoint = torch.load(new_CKPT_PATH)
#初始化模型参数
LPsam.load_state_dict(checkpoint)
LPsam.eval()
...
class Window(QWidget):
    ...
    def mouse_release(self, ev):
        ...
        sam_mask = medsam_inference(LPsam, self.embedding, box_1024, H, W)
