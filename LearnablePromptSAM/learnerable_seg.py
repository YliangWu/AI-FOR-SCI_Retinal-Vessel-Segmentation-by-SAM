...
class PromptSAM(nn.Module):
    def __init__(self, model_name, checkpoint, num_classes=2, reduction=4, upsample_times=2, groups=4,
        ...
        """.neck 是模型中的另一个组件，通常称为"neck"，它通常用于进一步处理从图像编码器中提取的特征。"neck" 层在图像分割模型中通常用于减小特征的空间分辨率或进行特征融合等操作，以帮助更好地执行分割任务。"""
    ...
    def forward(self, x):
        ...
        seg_out = self.decoder(out)  #输出：1,12,256,256
        seg_out = F.interpolate(seg_out, size=(self.img_size, self.img_size), mode="bilinear", align_corners=True)  #张量差值，修复图像分辨率 1,12,1024,1024
        return seg_out
...
if __name__ == "__main__":
    with torch.no_grad():
        model = PromptSAM("vit_b", "./weights/sam_vit_b_01ec64.pth").half().cuda()
