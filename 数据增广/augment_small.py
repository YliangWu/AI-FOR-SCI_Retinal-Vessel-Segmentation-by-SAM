...
def med_augment(data_path, name, level, number_branch, mask_i=True, shield=False):
    if mask_i:
        ...
        mask_path = f"{data_path}mask"
        output_path = f"{os.path.dirname(os.path.dirname(data_path))}/medaugment1/{name}/"
        out_mask = f"{os.path.dirname(os.path.dirname(data_path))}/medaugment1/mask/"
    else:
    ...
    for j, file_name in enumerate(os.listdir(image_path)):
        if file_name.endswith(".png") or file_name.endswith(".jpg"):
            ...
            if mask_i:
                mask = cv2.imread(f"{mask_path}/{file_n}.jpg")
                # print(f"{mask_path}/{file_n}.jpg")
...
def generate_datasets(data_path, dataset, seed, level, number_branch):
    ...
    # data_path = f"./datasets/{dataset}/transformed_training/"
...
def main():
    ...
    group.add_argument('--dataset', default='DRIVE')
    ...
    group.add_argument('--data_path', default=f"./datasets/Drive/extract_1/")
